...
local floor, ceil, GetTime, strformat = math.floor, math.ceil, GetTime, string.format
...
local function UpdateTimer(self)
	...
	if timeLeft <= 0 then
		...
		return timer:Hide()
	elseif timeLeft < addon.db.profile.timerPrecision and addon.db.profile.bigTimer then -- timerPrecision = 3
	...
	if timeLeft ~= timer.timeLeft then
		...
		if not true then -- need to make a user choice
...
local function SpawnIcon(self)
	...
	bigText.fontSize = db.bigTextSize
	bigText:SetFont(LSM:Fetch(LSM.MediaType.FONT, db.iconFont), bigText.fontSize, db.bigTextFlags)
	bigText:SetTextColor(1, 1, 1, 1)
	bigText:SetShadowOffset(db.bigTextShadowXOffset, db.bigTextShadowYOffset)
	--bigText:SetAllPoints(icon)
	bigText:SetPoint(db.bigTextPoint, icon, db.bigTextRelativePoint, db.bigTextXOffset, db.bigTextYOffset)
	...
	smallText.fontSize = db.smallTextSize
	smallText:SetFont(LSM:Fetch(LSM.MediaType.FONT, db.iconFont), smallText.fontSize, db.smallTextFlags)
	smallText:SetTextColor(1, 1, 1, 1)
	smallText:SetShadowOffset(db.smallTextShadowXOffset, db.smallTextShadowYOffset)
	--smallText:SetAllPoints(icon)
	smallText:SetPoint(db.smallTextPoint, icon, db.smallTextRelativePoint, db.smallTextXOffset, db.smallTextYOffset)
...
local function UpdateIcon(icon, texture, count, duration, expireTime)
	...
	icon.border:SetBackdropBorderColor(r, g, b, 1) -- 0 = removed borders.. cba doing a clean thing
